...
- `tools/stark_session_lib.ts` + `tools/stark_session.ts` — `/stark-session` data collector. Subcommands `start` and `end` return structured JSON; Claude renders the briefing/summary directly. Session-state, persona, alerts, skill-suggestions collectors hit pure-TS siblings; remaining Python shell-outs (`healer_canary.py`, `github_projects.py`) are scheduled for their own follow-up slices. Replaces the deleted `session_tui*.py` ANSI/box-drawing renderer.
...
- `tools/alert_delivery_lib.ts` + `tools/alert_delivery.ts` — TS-canonical alert emit + check. Same on-disk contract as the Python (alerts.jsonl + alert-{ts}.marker files in `~/.claude/code-review/`, same-second collision counter). Python `scripts/alert_delivery.py` kept alive because self_healer + healer_canary still import `emit_alert`; both producers target the same marker dir, so cross-language interop is automatic.
