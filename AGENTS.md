...
- `tools/stark_session_lib.ts` + `tools/stark_session.ts` — `/stark-session` data collector. Subcommands `start` and `end` return structured JSON; Claude renders the briefing/summary directly. Session-state, persona, alerts collectors hit pure-TS siblings; remaining Python shell-outs (`healer_canary.py`, `skill_router.py`, `github_projects.py`) are scheduled for their own follow-up slices. Replaces the deleted `session_tui*.py` ANSI/box-drawing renderer.
