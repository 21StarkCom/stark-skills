...
Gru preserves session worktrees and ownership after completion. Native worker observations use that worker's provider view; unavailable providers retain their own uncertainty. Opaque identities and leadership transfers still require complete discovery. Lifecycle changes require fresh Hermod observations. Unknown startups remain reserved; replacement retains pending merge locks and bases; termination requires matching session, surface, and PID evidence with `alive=false`. A retained merge settles only until the replacement attaches, or after cancellation froze an in-flight integration — never during an uncertain reconnect, intake, or implementation, and cancelling a working replacement keeps it resumable rather than verifiable. Verified workers release slots when freshly observed idle, dead, or confirmed retired. Overlapping declared files never block dispatch: workers edit separate worktrees and reconcile at the rebase before merge, serialized by the repository merge lock — sound only while the leader grants integration at the base branch tip it fetches and reads immediately before `integrate` (which takes the merge lock itself, so the lock cannot be held first, and whose `resource already owned: merge:<repo> (<run>/<task>)` refusal names the task to wait for), which `integrate` now enforces rather than trusting: before the transaction it fetches the base branch from origin (`--base-ref BRANCH`, else the default branch **origin itself reports** via `git ls-remote --symref` — never the local `refs/remotes/origin/HEAD`, which git writes at clone and then only on an explicit `git remote set-head`, so a checkout predating a default-branch rename would freeze the comparison on a branch nobody merges into and turn the whole guard off while every message reports it on) in the task's own repository and refuses unless the SHA is a commit that repository holds, is that branch's current tip, and contains every merge this engagement already verified onto the same branch — naming the current tip in the refusal, and failing closed (an unreachable origin, an unknown branch, an unresolvable default each refuse and say which, never falling back to a local ref). A task not in `review` refuses before that fetch, and `verify` refuses a PR whose base branch is not the one the grant was checked against. Without it the store validated the SHA's shape alone while `verify` only requires that base in the merged head's ancestry, so a foreign-repository SHA, a typo, or an hour-stale tip all passed. Verification uses invocation-owned `refs/gru/verification/<uuid>/*` without writing `FETCH_HEAD`, runs declared checks in order (each bounded by `checkTimeoutMs`, default 30 minutes), and removes only its disposable checkout. Cleanup errors remain visible without replacing a failed check. Late workers can attach during cancellation. Native Codex startup and complete briefing have been exercised with Hermod v0.17.0 (STARK-4911). Dependency release was demonstrated live on 2026-09-16: a dependent task stayed refused after its prerequisites merged and released only on Gru's own verification events. Full acceptance remains incomplete — bounded recovery is unproven, and native Codex leadership is still unevaluated because that run used a Claude leader. `receive` requires Hermod-confirmed delivery, so a Claude leader acks each inbound report first; `resume --limits-file` replaces operating limits that a leadership transfer left stale. Codex sessions yield while awaiting queued reports. Review fixes receive final-diff validation without automatic review loops. `takeover --file` repairs orphaned worker ownership only with a direct operator instruction plus complete fresh Hermod absence checks (every workspace's surfaces, saved sessions, terminal processes, and an independent OS PID probe whose signal termination retains a null status and refuses takeover; the replacement path must stay absent through the transaction); it fences the old token, preserves budgets, PR evidence, worktrees and merge ownership, never calls an unknown worker dead, keeps the pending assignment's exclusive resources until its replacement reserves or the retained merge settles, and admits a resumed stopped assignment after reconciliation. Contract: [operator takeover](skill/gru/references/operations.md#operator-takeover-when-runtime-records-are-gone). `attach` adopts the worktree Hermod actually placed (Claude `<repo>/.claude/worktrees/<ticket>`, Codex `<main checkout>/.worktrees/<ticket>`) instead of refusing, only for a linked worktree root of the same repository whose directory or branch names the ticket, that no other unswept task declares and no other task owns, and that no takeover fenced (identity refusals are named before git is read); it issues a new token so the worker must receive a fresh packet, records a `worktree-adopted` event with the git evidence, and every other mismatch still refuses. A Minion decides a re-brief with the DB-free `gru rebrief-check` command and rereads its latest accepted message after resumption. It reports replay ordering and checks leadership epochs; an unreadable baseline fails closed with an explicit recovery path. Packets carry at most 16 transfer receipts and preserve the byte-exact acknowledgement text separately from display indentation. The store requires discovery evidence for every changed leader. Packets are Hermod notes and acknowledgements are fresh progress messages, so the request reply deadline cannot strand intake. For sandboxed discovery, `resume` records a portable transfer receipt in the packet; a visible live old leader still refuses it. The packet and both runtime skills point to [the tested check contract](skill/gru/references/operations.md#deterministic-re-brief-check). Checkout mismatches remain plain notes, which `receive` identifies once addressed to the leader. Hermod attribution is advisory, so Gru's store token fence stays the authority. Contract: [worktree placement](skill/gru/references/operations.md#worktree-placement). `sweep [--run ID] [--apply]` (read-only dry run by default) releases a reservation only when Alfred reports its ticket `done`/`Closed` and no live Hermod peer is bound, never on elapsed time; integration grants, uncertain reconnects, and `reserved` launches in a `running` engagement stay held, and `--apply` needs no leader but records a revision-fenced `swept` event naming the proof. Contract: [proof-based sweep](skill/gru/references/operations.md#proof-based-sweep-of-dead-reservations).
