...
Gru preserves session worktrees when retiring completed workers. Native worker observations use that worker's provider view; unavailable providers retain their own uncertainty. Leadership transfers still require complete discovery. Lifecycle changes require fresh Hermod observations; unknown startup outcomes remain reserved, and a stale Hermod hook record for a session reported `alive=false` counts as termination. Verification fetches the reviewed PR head into private refs, runs declared setup/check commands in order (each bounded by the task's `checkTimeoutMs`, default 30 minutes), and removes only its disposable checkout. Late workers can attach during cancellation so Gru can interrupt their verified identities. Native Codex startup and complete briefing have been exercised with Hermod v0.17.0 (STARK-4911); full Gru acceptance remains incomplete. Codex sessions yield while awaiting queued reports. Review fixes receive final-diff validation without automatic review loops. A fully verified engagement releases its ticket, worktree, and identity ownership.
