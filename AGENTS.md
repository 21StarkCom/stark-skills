...
Gru preserves session worktrees when retiring completed workers. Lifecycle changes require fresh Hermod observations; unknown startup outcomes remain reserved. Verification fetches the reviewed PR head, runs declared setup/check commands in order, and removes only its disposable checkout. Late workers can attach during cancellation so Gru can interrupt their verified identities. Native Codex ticket launch and complete initial briefing depend on Hermod STARK-4911; unit checks do not establish live parity.
