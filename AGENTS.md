...
- `tools/forge_state_lib.ts` (pure) + `tools/forge_state.ts` (host CLI) — the `/stark-forge` pipeline **state manager**. Pure lib = clock/network/disk-free state machine (transition matrix, atomic `reconcileRunningStage` crash primitive, `recordOutput` registry semantics, `resolveChain`/`mergePointsFor`, `planPathFor`/`parsePlanSlug`, `renderStageCommand`, `requiresBaseSync`, `initializeRun`, `resumeTarget`); every mutating fn returns a new `RunState`, timestamps host-supplied. Host CLI = all disk I/O (`persistState`/`loadState`/`resolveLatest`/`listResumeCandidates` over `stateRoot()`, 0600) + 9 subcommands (`resolve`/`init`/`record-output`/`transition`/`get`/`abandon`/`summary`/`resume-target`/`driver-block`) + the `gh pr view` reader + repo-identity binding. Config section `forge_pipeline` (`getForgePipelineConfig()`, distinct from the review-routing `forge` section).
...
### Infrastructure
- `tools/stark_config_lib.ts` — full config reader (DEFAULT_* sections, per-section accessors, deep merge). Includes `forge_pipeline` (`DEFAULT_FORGE_PIPELINE` = `{history_keep_runs: 20, merge_timeout_s: 1800}`, `getForgePipelineConfig()`) — separate from the review-routing `forge` section, no symbol collision.
...
- `/stark-forge <intent|spec-path|plan-path> [--red-team] [--from STAGE] [--until STAGE] [--resume] [--dry-run] [--json]` — **the conductor** (`skill/stark-forge/SKILL.md` + `tools/forge_state{,_lib}.ts`). Chains the six stages below (8 with `--red-team`) in-session; **glue, not logic** — chain, merge points, per-stage command, and base-sync routing all resolve in `forge_state.ts`, the skill only invokes stage slash-commands + drives the CLI. Stages report on a `STARK_STAGE_SUMMARY` line (`standards/stage-completion-line.md`; forge invokes them without `--json`) → `record-output`. Artifacts merge at their last touching stage via `/stark-gh:pr-merge`; base sync fires only for new-artifact stages (`requiresBaseSync`). `--resume` reconciles a crashed stage (one atomic `crashed`-attempt primitive) → closed action enum (`reinvoke|advance|complete|merge_only|abandon`). In-session by default; `STARK_FORGE_DRIVER=1` prints operator command blocks instead. State under `stateRoot()/history/forge/<slug>/<run-id>/state.json` (0600, repo-bound). Config section `forge_pipeline`. Spec `docs/specs/2026-07-19-stark-forge-spec.md`.
