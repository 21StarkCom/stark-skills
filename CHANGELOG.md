...
- **cc-user: `refresh` re-captures the active seat instead of skipping it, and a seat mismatch no longer destroys the credential (STARK-246, PR #862).** Both defects were found by exercising the command shipped in #861 against the real fleet. (1) Skipping the active seat looked safe but is how a profile rots into a revoked token needing a browser login: the live `Claude Code-credentials` item is global and the running CLI rotates it on its own schedule, so the stored copy dies while its own `expiresAt` still reads hours ahead — `classifyRefresh` reports `fresh` and every check calls a dead profile healthy. New `activeCopyIsStale` compares the stored refresh token against the live one (a fact, not an inference from timestamps that rotation never updates) and, on a mismatch, copies the live blob into the profile — no network, no rotation, and it only ever overwrites an already-dead credential. New `recaptured` / `would-recapture` statuses; the incoherence guard still applies, so a live item holding another account's token is never bottled. (2) `seatMismatch` now warns and **still writes**: skipping the write discarded the token that had just rotated and killed the profile, which is the outcome this module exists to prevent. The sent token is dead on arrival, so the response is the only live credential for that seat — the registry *label* drifted, not the credential, and `add <name>` re-labels it. For a rotating credential, "refuse to write" is never the safe default.
