...
- `gru resume --limits-file` attributes a parse failure to the flag and the path. A bare `JSON.parse` surfaced only a parser offset into an unnamed buffer, naming neither, which defeated the stated purpose of reading the file before the slow Hermod transfer check.
