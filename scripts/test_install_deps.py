...
class TestCheckPythonDeps:
    def test_warns_when_venv_missing(self, scratch_repo: Path):
        ...
        for pkg in ("PyJWT", "requests", "anthropic", "google-auth"):
    ...
    def test_reports_missing_packages(self, scratch_repo_with_venv: Path):
        # The fresh venv has none of PyJWT/requests/anthropic/google-auth installed.
    ...
    def test_reports_installed_packages(self, scratch_repo_with_venv: Path):
        # Install all deps into the scratch venv and re-check.
    ...
    def test_anthropic_vertex_deps_are_checked_alongside_legacy_deps(
        self, scratch_repo_with_venv: Path,
    ):
        """Regression guard: ``anthropic`` and ``google-auth`` are both
        required by the SDK dispatch path. If a future refactor drops either
        from the pair loop, forge fix-dispatch breaks on fresh installs. The test
        above (``test_reports_missing_packages``) already covers this
        via the full package assertion, but we keep this narrower test
        so the signal is unambiguous when it fires."""
        result = _run_dep_check(scratch_repo_with_venv)
        assert "anthropic" in result.stdout, (
            "anthropic package is not being checked by check_python_deps"
        )
        assert "google-auth" in result.stdout, (
            "google-auth package is not being checked by check_python_deps"
        )
