...
The happy path from idea to shipped code, fully autonomous after the first two steps:

```mermaid
graph TD
    subgraph START [" "]
        S["/stark-session start<br><i>context · git state · briefing</i>"]
    end

    subgraph DESIGN ["Design"]
        B["Brainstorm<br><i>explore requirements · design options</i>"]
        W["Write spec.md"]
        RP["/stark-review-plan<br><i>3 LLMs × 7 domains</i>"]
        FIX1["Fix spec from findings"]
    end

    subgraph PLAN ["Plan → Tasks"]
        PT["/stark-plan-to-tasks<br><i>quality gate → decompose → validate</i>"]
        GH["GitHub Issues created<br><i>phased · story points · risk labels</i>"]
    end

    subgraph EXECUTE ["Autonomous Execution"]
        PE["/stark-phase-execute<br><i>per task:</i>"]
        IMPL["Branch → Implement → Test"]
        PR["Create PR"]
        CR["/stark-review<br><i>3 LLMs × 6 domains</i>"]
        FIXR{"Findings?"}
        APPLY["Apply fixes · re-review"]
        MERGE["Squash-merge → close issue"]
        NEXT{"More tasks?"}
    end

    subgraph SHIP ["Ship"]
        REL["/stark-release<br><i>CHANGELOG · tag · GitHub Release</i>"]
        ED["/stark-session end<br><i>tests · cleanup · push</i>"]
    end
    ...
    style S fill:#047857,color:#fff,stroke:none
    style B fill:#7c3aed,color:#fff,stroke:none
    style W fill:#7c3aed,color:#fff,stroke:none
    style RP fill:#1e40af,color:#fff,stroke:none
    style FIX1 fill:#7c3aed,color:#fff,stroke:none
    style PT fill:#1e40af,color:#fff,stroke:none
    style GH fill:#f59e0b,color:#1a1a1a,stroke:none
    style PE fill:#1e40af,color:#fff,stroke:none
    style IMPL fill:#047857,color:#fff,stroke:none
    style PR fill:#047857,color:#fff,stroke:none
    style CR fill:#1e40af,color:#fff,stroke:none
    style FIXR fill:#7c3aed,color:#fff,stroke:none
    style APPLY fill:#dc2626,color:#fff,stroke:none
    style MERGE fill:#047857,color:#fff,stroke:none
    style NEXT fill:#7c3aed,color:#fff,stroke:none
    style REL fill:#f59e0b,color:#1a1a1a,stroke:none
    style ED fill:#047857,color:#fff,stroke:none
```

**Color legend:** 🟢 Action  🔵 Skill (multi-agent)  🟣 Decision/creative  🟡 Artifact  🔴 Fix loop
