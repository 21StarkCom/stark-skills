...
- `tools/alert_delivery_lib.ts` + `tools/alert_delivery.ts` — TS-canonical alert emission + check. Same on-disk contract as the Python (`~/.claude/code-review/alerts.jsonl` + `alert-{ts}.marker` files in the same dir, same-second collision counter). `scripts/alert_delivery.py` kept alive because `self_healer.py` + `healer_canary.py` still import `emit_alert` in-process; the shared marker dir makes both producers naturally interoperable. CLI: `--check [--json]`.
