...
- `scripts/session_state.py` — persistent session state management. **Mid-migration to TS** — pure-TS implementation lives at `tools/session_state_lib.ts` + `tools/session_state.ts`; the Python remains authoritative only because `scripts/context_compactor.py` still imports `SessionState` as a class. Both get deleted in the context-compactor port.
...
- `tools/session_id_lib.ts` + `tools/session_id.ts` — TS port of `scripts/session_id.py`. Three-tier resolver: `CLAUDE_SESSION_ID` env > `~/.claude/projects/` newest-mtime marker scan > uuid4. Consumed by `tools/emit_queue_lib.ts` and `tools/session_state_lib.ts`. Python kept only because `session_state.py` still imports it from `context_compactor.py`'s side; all three Python files go in the context-compactor port.
- `tools/session_state_lib.ts` + `tools/session_state.ts` — TS port of `scripts/session_state.py`. Same path-traversal sanitization, same on-disk JSON shape under `~/.claude/code-review/sessions/{sanitized-id}.json`, same git-derived defaults (`gitBranch`, `gitRepo` with GitHub URL normalizer). CLI surface: `[--session-id ID] [--json]` matches Python, plus a new `set --field <name|start_head|last_checkpoint> --value VAL` subcommand that replaces the inline `python3 -c "from session_state import …"` snippets in `/stark-session` SKILL.md.
