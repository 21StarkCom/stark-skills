...
Multi-agent PR code review system. Claude and Codex are enabled by default, with Gemini kept as an optional config-controlled competitor. Hierarchical config (global → org → repo). Self-improving prompts via review history analysis.
...
- `skill/` — all skills (`skill/stark-*/SKILL.md`, 28 skills), installed as symlinks to `~/.claude/skills/`
...
- `scripts/design_to_plan_dispatch.py` — generic generate-and-cross-review dispatch for the enabled agents
- `scripts/autopilot_dispatch.py` — tournament-based autonomous implementation (all enabled agents compete in worktrees)
