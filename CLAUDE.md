...
- `/stark-red-team-design <path> [--source-spec PATH] [--model ID] [--dry-run] [--no-pr-comment]` — adversarial committee challenge of a design doc (5 personas × 1 round, default `gpt-5.5-pro`); writes `<design>.red-team.md` sidecar and always lands findings on a PR — **opens one (pushing the design doc + sidecar) if none exists**, else edits in place; the PR + findings comment are authored by the GitHub App matching the run's agent (`claude*`→stark-claude, `gpt*`/`o*`→stark-codex, `gemini*`→stark-gemini; codex by default); challenge-only, no fix loop
