...
- `tools/session_id_lib.ts` + `tools/session_id.ts` — TS port of `scripts/session_id.py` (kept alongside the Python until `session_state.py` ports too — `session_state.py` still imports the Python resolver). Three-tier resolver: `CLAUDE_SESSION_ID` env > `~/.claude/projects/` newest-mtime marker scan > uuid4. `tools/emit_queue_lib.ts` delegates to this lib (no more inline partial copy with `// Skip projects-dir` debt), so every TS producer sees the same session ID the Python session-state machine does.
