...
2. For the next `pending`/re-entered stage, forge transitions it `→ running` and **prints** (a) the exact stage command from the interfaces §2 table with resolved paths, and (b) the exact `forge_state.ts record-output …` followed by `forge_state.ts transition … --to done|failed …` invocations the operator will run to report the result. It then stops.
