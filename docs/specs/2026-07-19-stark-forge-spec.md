...
- `running → done` requires the stage completed **and**, if the stage is a merge point's `after_stage`, **all** of its artifact PRs (`prs`) merged **and** no fold PR (`fold_prs`) left open (for `plan-to-tasks`, requires only that issue creation completed and `artifacts.issue_numbers` was recorded — there is no PR).
...
|-----------|------|---------------|
| `init` | `--slug --run-id --chain <csv> --red-team <bool> --created-at <iso>` | Computes `merge_points`, writes the initial `state.json` (all stages `pending`); prints the run object. |
| `transition` | `--slug [--run-id] --stage --to <status> [--prs <csv>] [--fold-prs <csv>] [--gate-reason --gate-detail] [--artifact-spec-path --artifact-plan-path --artifact-plan-slug --artifact-issue-numbers <csv>] --at <iso>` | Applies one legal transition atomically; prints the updated stage record. Exits non-zero (throws) on illegal transition. |
| `get` | `--slug [--run-id]` | Prints the full run object. |
| `resume-target` | `[--slug]` + injected PR-state reader | Prints `{run_id, slug, target_stage, action: reinvoke \| advance \| complete, reconciled?: boolean}` — the next stage to run after reconciliation. |
...
`--json` emits exactly one JSON object on **stdout** (all narration on stderr); the same object is produced by `forge_state.ts summary`. Schema:
...
| `status` | `completed \| halted \| failed` | Overall run status: `completed` = last chain stage `done`; `halted`/`failed` = a stage is halted/failed. |
...
1. **Stage has one or more `prs`** (write-spec/review-spec/red-team-spec on the spec PR; spec-to-plan/review-plan/red-team-plan on the plan PR; copilot on one or more impl PRs) → query **each** read-only (`gh pr view --json state,mergedAt`). **All merged** → the stage completed and its artifact(s) landed; push `{outcome: crashed}` and `transition running → done` (marked `reconciled`), advance to the next stage. **Any open/unmerged** → push `{outcome: crashed}`, `transition running → failed` with `gate.reason = "reconciled_after_crash"`, then `failed → running` re-invokes the stage via its create-or-adopt idempotent entry point (adopts the existing PR(s), commit-on-top; forge re-merges only the still-open ones — duplicate side effects are impossible by the stage skills' existing contract). For a red-team stage, an open **fold** PR (`fold_prs`) is surfaced but does not by itself force challenge re-invocation — it routes to the `fold_pr_open` halt at the artifact merge point.
