...
- **Auth tokens ↔ Keychain**: `/data/bootstrap_token` AND `/data/prune_token` are generated by the server (Phase 1) and mirrored to the macOS Keychain by the bootstrap helper as TWO scoped services: `stark-observability-bootstrap-token` (UI + main-API Bearer) and `stark-observability-prune-token` (retention-listener-only). The prune CLI (Phase 7) reads ONLY the prune service. There is no `stark-observability-token` service anywhere in the codebase. Scripts that need Bearer auth read the appropriate scoped service directly via `security find-generic-password -s stark-observability-bootstrap-token -w` or `… -s stark-observability-prune-token -w` (NEVER the unscoped legacy name).
...
- **Phase 1**: `docker compose -f tools/observability_server/docker-compose.yml down`; `rm -rf ~/.claude/code-review/observability/`; `launchctl unload tools/observability_server/launchd/com.aryeh.observability.hostinfo.plist`; `security delete-generic-password -s stark-observability-bootstrap-token` (if installed); `security delete-generic-password -s stark-observability-prune-token` (if installed). No dispatcher impact.
...
## 7. Errata — residual review findings (must address during implementation)

The `/stark-review-plan` fix loop ran 3 review-fix rounds + a final
review-only round. 81 wing patches were auto-applied (29 + 24 + 28). A
final 22 codex findings (1 critical, 12 high, 9 medium) remained
unresolved after the 3-round budget; they cluster into the corrections
below. Implementers MUST treat these as P0 fixes to the plan before
writing code for the affected phase.

### E1 (CRITICAL, viability) — Browser bootstrap cannot load the UI

**Affected:** Phase 4 Task 1 (auth middleware), Phase 5 Task 2 (UI mount).

**Problem.** The auth middleware exempts only `/api/health/probe`,
`/api/auth/bootstrap`, and `/api/auth/exchange`. The browser's first
load of `http://127.0.0.1:7700/#b=<code>` then needs `GET /index.html`
and its asset bundle without a cookie — and 401s instead.

**Required fix.** Exempt the static bootstrap shell — `GET /`,
`/index.html`, and all immutable hashed assets under `/assets/*` — from
the auth middleware. Keep every `/api/*` route protected. Add a
Playwright test that verifies a cold browser load with no cookie
renders the bootstrap shell and posts the fragment code to
`/api/auth/exchange` successfully.

### E2 (HIGH, completeness + sequencing) — Final child output races past `subagent_end`

**Affected:** Phase 2 Task 3 (`attachChild`), Phase 2 Task 6
(`wrapRunProcess`), Phase 6 Task 2 (dispatcher integration).

**Problem.** `attachChild` is synchronous and its stdout/stderr chunk
pumps are fire-and-forget. `runProcess` returns when the child exits
but does not await pending UDS-write acknowledgements. Phase 6 calls
`endSubAgent` immediately after `runProcess` returns, so the last
hundreds of milliseconds of output can land in the JSONL *after* the
`subagent_end` event and confuse the index writer's idempotency.

**Required fix.** `attachChild` returns a `{ drain: () => Promise<void> }`
handle. `wrapRunProcess` awaits both `child.stdout`/`child.stderr`
`"end"` AND `handle.drain()` before resolving. `endSubAgent` is only
invoked after `runProcess` resolves. Add a load test that bursts 5 MB
of stdout in the last 100 ms before exit and asserts every byte lands
before `subagent_end`'s `seq`.

### E3 (HIGH × 3) — Phase-execute lease across fresh shells

**Affected:** Phase 6 Task 4 (`/stark-phase-execute` integration).

**Problem.** SKILL.md bash blocks may run in fresh shells; `$SESSION_ID`
is empty in later blocks. Lease-touching commands then write to the
wrong path. Existing exec-child wrapper runs `lease.tick` once before
`execve`, so a long child outlives its lease.

**Required fix.** Every `phase_execute_observability.ts` subcommand
resolves the session id itself via `tools/session_id.ts` unless
`--session-id` is passed. The `exec-child` subcommand becomes a
supervising wrapper: fork-exec the child, refresh `lease.tick` every
30 s until child exit, record/remove child PID state, forward signals,
and exit with the child's exit code. Integration test runs each SKILL
block in a fresh shell and asserts the lease stays fresh end-to-end.

### E4 (HIGH × 3) — Prune CLI auth contradiction

**Affected:** Phase 7 Task 1 (prune CLI auth).

**Problem.** Phase 4 accepts the prune token ONLY on the retention
listener (port 7701) for `POST /api/internal/retention/notify`. Phase 7
Task 1 attempts `GET /api/runs?status=crashed` on the main API with
the prune token — which is rejected.

**Required fix (pick one):**

- **Option A.** The prune CLI reads BOTH scoped credentials. The
  bootstrap token authenticates the read against `GET
  /api/runs?status=crashed` on the main API; the prune token continues
  to authenticate the `POST /api/internal/retention/notify` writes on
  the retention listener.
- **Option B.** Add a `GET /internal/retention/crashed-runs` endpoint
  on the retention listener (port 7701) authenticated by the prune
  token; the prune CLI consumes this instead of the main API. No
  cross-token surface.

Option B is preferred — it preserves the strict prune-token isolation.

### E5 (HIGH × 3) — Rewrite gate stuck after `rename(2)` failure

**Affected:** Phase 7 Task 3 (prune rewrite flow), Phase 3 Task 4
(tailer gate), Phase 4 Task 2 (retention notify schema).

**Problem.** `action: "pre-rename"` sets `spool_files.rewrite_pending = 1`
and the tailer skips that file. If `rename(2)` fails after the
pre-rename POST, the plan logs the error but leaves `rewrite_pending = 1`
forever — the tailer silently drops events for that file from then on.

**Required fix.**

1. Add a new `action: "abort-rewrite"` payload; on `rename(2)` failure
   the prune CLI immediately POSTs this and FAILS the whole file if
   the abort itself fails.
2. Make `action: "update-mtime"` a must-complete roll-forward: the
   prune CLI persists a host-side pending-update journal at
   `~/.claude/code-review/observability/.pending-rewrites/{run_id}-{rotation_index}.json`
   and retries with bounded backoff until either `update-mtime` or
   `abort-rewrite` succeeds.
3. At server startup AND prune CLI startup: scan
   `spool_files WHERE rewrite_pending = 1`, compare actual file size to
   `rewrite_pending_size_bytes`, and finish whichever transition the
   journal points to.

### E6 (HIGH, security) — Redaction does not cover structured payloads or base64 chunks

**Affected:** Phase 2 Task 5 (`redact(text)`), Phase 2 Task 4 (base64
chunk path), Phase 6 (finding/summary emissions).

**Problem.** `redact(text)` operates only on UTF-8 strings. Structured
`payload` objects (findings, summaries, errors) and base64-encoded
chunks bypass it.

**Required fix.** Add `redactJson(value: unknown): unknown` to the
emit-lib that recursively walks objects, redacting every string leaf
(with depth cap 32 and per-string size cap 1 MB). For base64 chunks:
decode → `redact()` → re-encode; if the decoded content is not valid
UTF-8 and exceeds 1 MB, replace the whole chunk with a `chunk_truncated`
sentinel rather than emit unredacted bytes. Negative test cases for
every built-in pattern in both flat text and nested payload positions.

### E7 (HIGH, sequencing + completeness) — Pre-rename before tailer scan

**Affected:** Phase 3 Task 4, Phase 7 Task 3.

**Problem.** Pre-rename only updates an existing `spool_files` row. On
a server restart that hasn't yet scanned a terminal run, the prune CLI
can POST pre-rename and the server has no row to update.

**Required fix.** The `pre-rename` handler requires an existing
`spool_files` row and returns 409 `{ code: "scan_pending" }` otherwise.
The prune CLI retries after `min(5 s, server-scan-lag)` with exponential
backoff, or calls a new `POST /internal/retention/scan-now` endpoint
that triggers an immediate filesystem scan of the named run.

### E8 (MEDIUM, security) — Container hardening

**Affected:** Phase 1 Task 3 (Dockerfile + compose).

**Required fix.** The Dockerfile creates and runs as an unprivileged
UID (`adduser -D -u 10001 observability`). The compose service sets:

- `cap_drop: ["ALL"]`
- `security_opt: ["no-new-privileges:true"]`
- `read_only: true` (root FS)
- explicit `tmpfs:` mounts for `/tmp`
- `/data` and `/audit` remain writable as named volumes

Phase 1 verification: `docker inspect` confirms each setting; the
server still serves `/api/health/probe`.

### E9 (MEDIUM, completeness) — Audit log mount missing from Phase 1

**Affected:** Phase 1 Task 1 (`ensureRoot`), Phase 1 Task 3 (compose).

**Required fix.** Phase 1 `ensureRoot()` creates
`~/.claude/code-review/observability/audit/` with mode 0700. Phase 1
docker-compose adds the bind mount:
`~/.claude/code-review/observability/audit:/audit`. Phase 4 Task 7 then
writes `audit.jsonl` to that mount; deleting `observability_index` does
not affect audit log survival. Phase 1 verification asserts the mount
exists and is writable from inside the container.

### E10 (MEDIUM, sequencing) — Daemon dies before first run-heartbeat

**Affected:** Phase 2 Task 9 (`startRun`), Phase 4 Task 3 (liveness sweep).

**Required fix.** `run_start` events carry `tracked_parent_pid` AND
`writer_daemon_pid`. The index writer (Phase 3) immediately populates
`runs.parent_pid` from `run_start` rather than waiting for the first
`run_heartbeat`. The liveness sweep also handles
`last_heartbeat_at IS NULL`: any run older than `daemon_readiness_timeout`
(30 s) with no heartbeat is treated as crashed-before-heartbeat.

### E11 (MEDIUM, security) — Bearer token examples expose secrets in argv

**Affected:** every plan example that uses `curl -H "Authorization:
Bearer $TOKEN" …`.

**Required fix.** Replace every Bearer-in-argv example with one of:

- cookie-file auth: `curl -b "$COOKIE_FILE" …` (after running the
  bootstrap helper); this is the default form for all docs.
- A small TS helper `tools/observability_curl.ts` that reads the
  Keychain and sends the request without putting the token in argv.

For CLI-script paths that genuinely need the Bearer (e.g., the prune
CLI), write the header to a 0600 file and pass via `curl -K <file>`.

### E12 (MEDIUM, completeness/viability) — Fresh-install `statfs` before `ensureRoot`

**Affected:** Phase 2 Task 9 (`startRun`).

**Required fix.** `startRun` calls `ensureRoot()` BEFORE the
`fs.statfs(spoolDir)` low-disk preflight. The current ordering can
fail with ENOENT on first run.

### E13 (MEDIUM, completeness) — Pending-rewrite recovery

**Affected:** Phase 7 Task 3, Phase 3 startup.

**Required fix.** See E5 — the pending-rewrite journal and the
startup-scan recovery satisfy this finding as well.

---

The PR description for this branch references this Errata section so
reviewers know exactly which plan items still need implementation
attention before code is written for the corresponding phase.