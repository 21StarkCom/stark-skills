...
- Each loop iteration = 1–2 red-team calls (2 when findings exist, for stability) + optional design regen + optional inner design-review loop. At `max_rounds=2` worst case: 4 red-team calls + 2 regens + 2 inner review loops. `per_run_budget_usd` defaults to `$15.00` for `gpt-5.5-pro` (the v1.1 default; canonical value lives in §6). The earlier `$10.00` default targeted `o3`/opus pricing and is below the pro-tier envelope.
...
| Red team output is not valid JSON OR is empty OR is missing/malformed `findings` field | **(rt3, v1.1):** No longer fails open. Surface as a generic `error` on the `RedTeamResult` so the orchestrator routes to a degraded/halted path. The full model output is preserved on the same dataclass in `raw_output` for debug, but is **not** echoed into the `error` string — that string lands in audit logs and run state, where attacker-controlled spec/diff content embedded by the model could otherwise leak. The earlier "treat as zero findings = clean" behavior masked model outages and prompt drift as successful clean reviews. Callers must use `derive_status(result)` (which checks `error` first) rather than deriving status from finding counts alone. |
