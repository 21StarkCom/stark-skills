...
5. **Driver-mode command-block renderer + `driver-block` CLI subcommand**
   ...
   - Interfaces — **Consumes:** `renderStageCommand`, `resumeTarget`, `requiresBaseSync`. **Produces:** `renderDriverBlock(state: RunState, target: ResumeTarget): string` + the `driver-block` CLI subcommand.
