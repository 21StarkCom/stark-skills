...
- **Normative spec (pinned):** `docs/specs/2026-07-19-stark-forge-spec.md` at its PR-#736 landed revision (`8072cf3`). Every §-reference in this plan (§1 CLI + invalid-combination tables, §2 command table, §4 threading contract, §5 state schema, §6 transition matrix, §7 CLI contract, §9 summary schema, success criteria 1–7) resolves to that document — its tables ARE the acceptance fixtures; read the referenced section before implementing each task.
...
4. **`recordOutput` patch semantics + the one-owner PR registry**
   ...
   - Test: `record-output patch semantics: unions, write-once scalars, registry validation` (test #22) — includes `issue_numbers` union-dedup incremental persistence, write-once scalar `artifact_conflict`, `adoption_mismatch` on a continuation-stage divergent PR, **opening-stage incremental union across a crash (copilot reports PR 1, crashes, re-reports [1,2] → registry [1,2], no mismatch)**, and **registry seeding by a review stage in spec-path and plan-path chains through their merge gates**.
...
3. **`skill/stark-forge/SKILL.md`**
   - What: thin orchestrator — `## Help` block → `standards/help.md`; frontmatter `name: stark-forge`; resolvable tool refs (`tools/forge_state.ts`). Flow: call `forge_state.ts resolve <argv>` (validation + resolution + dry-run bridge) → `init` from the resolved descriptor → per stage:
     ...
     - execute the stage's `/stark-...` command in-session (plan-to-tasks carries `--plan-slug <recorded-plan-slug>`; or, in driver mode, print the Phase 5 driver block and stop for the operator);
...
- **`forge_state.ts` ↔ `/stark-gh:pr-merge`:** one invocation per artifact PR. Forge is the single deadline owner, enforced **by the executor that invokes the merge** (never a shell `timeout` wrapper — the merge is a slash-command invocation, not a host executable): in-session, the orchestrator tracks elapsed time against `merge_timeout_s` (from `getForgePipelineConfig()`); in driver mode, the printed block annotates the deadline and the operator reports expiry. Expiry → `transition running→halted --gate-reason merge_timeout`, retried on resume via `merge_only`.
