...
2. **State-mutating subcommands**
   - What: `init`, `record-output`, `transition`, `get`, `abandon`, `summary` per spec §7; JSON stdout; injected PR-state reader (real `gh pr view --json state,mergedAt` wrapper, injectable for tests). Timestamps `--at` host-supplied. Each wraps the corresponding pure-lib function and persists the returned `RunState` via `persistState` (Phase 3). `init` generates the `run_id`, resolves + records the canonical repo identity and the default branch (fail-fast — an unresolved/missing remote aborts with **no** state write), calls `initializeRun`, and persists the first `state.json`. `record-output` accepts `--artifact-issue-numbers <csv>` (spec §7) — **no `--issue-creation-complete` flag**; recording `issue_numbers` is the completion marker.
...
5. **Driver-mode command-block renderer**
   - What: **two-step protocol** — the block first emits the stage command; the operator runs it and returns the stage's **documented structured completion payload** (the §4 fields — PR numbers, generated paths, slug, issue numbers), which the block parses to fill `record-output` before emitting the final `transition` (carrying `--from`). Post-stage values are never pre-rendered as unresolved placeholders requiring manual editing. **Every `reinvoke`/`merge_only` block opens with the compare-and-set re-entry** (`transition --from failed|halted --to running`) — the Phase 4 task 1 re-entry ownership rule — so everything after it runs against a `running` stage. Then, for a **new-artifact stage** (spec-to-plan/plan-to-tasks/copilot), the block prints the base-sync (`git switch <state.default_branch>` + `git pull --ff-only`, reading the host-recorded default branch) and the `transition running→failed --gate-reason base_sync_failed` fallback — re-entry included, so a retried new-artifact stage never skips its sync; for a merge-point stage the block inserts fold-check + per-PR `timeout <merge_timeout_s> /stark-gh:pr-merge` + `--merges` recording + the `merge_timeout` halt fallback; the `merge_only` block omits the stage command. Same-artifact stages omit the base-sync lines.
   - Interfaces — **Consumes:** `renderStageCommand`, `resumeTarget`. **Produces:** `renderDriverBlock(state: RunState, target: ResumeTarget): string`.
   - Test: `driver mode emits exact command blocks and advances only on reported transitions` (#21) — asserts new-artifact stages carry the base-sync lines and same-artifact stages do not; plus a full driver-mode walkthrough starting from intent that carries synthetic structured completion payloads through every stage, proving `record-output`/`transition` render deterministically from the parsed payload; the base-sync line uses the recorded `state.default_branch` (asserted with a non-`main` default).
...
3. **`skill/stark-forge/SKILL.md`**
   - What: thin orchestrator — `## Help` block → `standards/help.md`; frontmatter `name: stark-forge`; resolvable tool refs (`tools/forge_state.ts`). Flow: call `forge_state.ts resolve <argv>` (validation + resolution + dry-run bridge) → `init` from the resolved descriptor → per stage:
     ...
     - **if the stage is `spec-to-plan`, `plan-to-tasks`, or `copilot`**: base-sync as the first act (`git switch <state.default_branch>` + `git pull --ff-only`, using the host-recorded default branch); on failure `transition running→failed --gate-reason base_sync_failed` and stop. **Same-artifact stages (review-spec, red-team-spec, review-plan, red-team-plan) and write-spec skip the sync** and run against the shared PR branch;
     ...
     - **at a merge point only**: merge each artifact PR via `/stark-gh:pr-merge` wrapped in a host `timeout <merge_timeout_s>` (from `getForgePipelineConfig()`), recording each `--merges` result; **on timeout expiry → `transition running→halted --gate-reason merge_timeout` and stop** (resume retries via `merge_only`);
...
- **`forge_state.ts` ↔ `/stark-gh:pr-merge`:** one invocation per artifact PR, wrapped by the host in `timeout <merge_timeout_s>` (resolved from `getForgePipelineConfig()`, exposed to both in-session and driver flows); expiry → `transition running→halted --gate-reason merge_timeout`, retried on resume via `merge_only`. Forge is the single deadline owner.
