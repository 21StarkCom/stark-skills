...
1. **Extend `lib/types.ts`** with the full schema. `schemaVersion: 1` is a literal type. `CleanupSkipReason` is a union of the safety/discovery reasons:
   ...
   **`CleanupPlan` top-level fields (enumerated exhaustively):**
   ...
   interface CleanupPlan {
     ...
     authUser: string;
     crossRepo: boolean;
     mode: "pr" | "branch" | "audit" | "stale" | "mixed";  // "stale" = --stale-days alone (§2 resolution 12 / plan-review H37)
     flags: CleanupFlags;
     protected: {
...
1. **Cross-cutting gate helpers** (plan-review H30/H36/H42 — extracted from old Phase 3 Task 5's interstitial block to land BEFORE the candidate-mode tasks so they call into shared helpers rather than re-implementing inline). Add three pure helpers in `gh_cleanup_preflight.ts` (or a small `gh_cleanup_gates.ts` if preferred — both consumers stay inside Phase 3, no `lib/` addition):

   - `isForkPr(pr: CleanupPr): boolean` — `pr.head.repo === null` OR `pr.head.repo.full_name !== pr.base.repo.full_name`. The null case covers deleted-fork (plan-review H11) — treated as fork because identity cannot be verified.
   - `verifyPrHeadIdentity(pr: CleanupPr, currentRemoteSha: string | null): "ok" | "branch-reused" | "gone"` — per-state baseline (per the matrix below). For merged/closed PRs where the ref still points at `pr.head.sha`, ALSO query `gh api repos/{owner}/{repo}/pulls?head=<owner>:<branch>&state=open`; non-empty → return `"branch-reused"` (plan-review C1, supersedes the unsound committerDate check).

     | PR state | Baseline (currentRemoteSha must be) |
     |---|---|
     | `merged` | `pr.head.sha` OR `null` (404). Other value → `"branch-reused"`. |
     | `closed` (not merged) | `pr.head.sha` OR `null`. Other → `"branch-reused"`. |
     | `open` (stale) | `pr.head.sha`. Other or `null` → `"branch-reused"` (404 should not happen for open PRs). |

   - `isProtected(branch: string, protected: CleanupPlan["protected"]): boolean` — lowercase-fold both sides; check `protected.branches` set membership (already includes hardcoded `[main, master, develop]` ∪ `defaultBranch` ∪ snapshotted `currentHead` from Phase 2 Task 3).

   These three helpers are the single source of truth for fork detection / branch-reuse / protection across **every** candidate builder (Tasks 2–5 below). Add `cleanup_gates.test.ts` covering: deleted-fork null head.repo → fork; mixed-case `Main` → protected; merged-PR with open-PR-on-same-head-ref → branch-reused; merged-PR with 404 ref → ok-gone.

2. **PR mode** (eligibility matrix per §2 resolution 8). For each `--pr N`, call `gh.ts:fetchCleanupPr(repo, n)` → state, `merged_at`, `head.ref`, `head.sha`, `head.repo.full_name`, `base.repo.full_name`, `updated_at`, `isDraft`, `author.login`. Build `CleanupAction` with `source.type: "pr"`.
   ...
   404 → `skip: true, skipReason: "pr-not-found"` on the action; do NOT exit immediately — plans with mixed valid/not-found PRs must still be written. The all-not-found → `EXIT_BAD_ARGS` decision lives in execute end-of-run precedence (Phase 1 Task 2), gated on `summary.skipReasons["pr-not-found"] === summary.candidates && !flags.allowNoPrMatch`.

   **Gate application:** for each eligible (non-skipped) action, call the Task 1 helpers in this order:
   - `isForkPr(pr)` → if true, `skip: true, skipReason: "fork"` (covers deleted-fork null head.repo per plan-review H11).
   - `isProtected(action.branchName, plan.protected)` → if true, `skip: true, skipReason: "protected"`.
   - `verifyPrHeadIdentity(pr, gh.ts:getRemoteHeadRef(repo, head.ref))` → if `"branch-reused"`, `skip: true, skipReason: "branch-reused"`; if `"gone"`, set `remote.exists = false` (proceed to local cleanup); if `"ok"`, record resolved SHA in `remote.headSha` for the Phase 4 drift check.

   Add `preflight_open_pr.test.ts` for the open-PR skip; `preflight_branch_reused.test.ts` and `preflight_null_head_repo.test.ts` and `preflight_merged_gone_ref.test.ts` already covered by `cleanup_gates.test.ts` (Task 1) — keep one happy-path PR-mode integration test (`preflight_pr.test.ts`) that exercises the matrix + gate-helper invocation in sequence.
   ...
   **Transient gh failures during preflight resolution** (plan-review H12). Any non-404 HTTP failure or network error from `gh.ts` during candidate resolution (`fetchCleanupPr`, `listClosedPulls`, `listOpenPulls`, `getRemoteHeadRef`) → exit `EXIT_UNRECOVERABLE` with a stderr line naming the failing call and the affected source (e.g. `"--pr 3: gh fetchCleanupPr failed (502 Bad Gateway). Aborting before plan write."`). Do NOT write a partial plan, do NOT continue with fewer candidates. The operator re-runs once the transient issue clears.
   ...
   _(Local-branch identity check — plan-review C44/C2 — requires `action.local` and is therefore performed in Task 7 below, not here. Already-clean short-circuit similarly runs in Task 8 after local metadata is populated.)_

3. **Branch mode.** For each `--branch NAME`, bypass PR lookup. **In same-repo mode**: check remote presence + SHA via `lsRemoteHeads(origin)`; check local presence + OID via `localBranchExists`/`localBranchOid`. **In cross-repo mode** (`plan.crossRepo === true`): resolve remote presence + SHA via `gh.ts:getRemoteHeadRef(plan.repo.nameWithOwner, name)` — NOT `lsRemoteHeads(origin)`, which would query the wrong remote (§2 resolution 13 inferred; plan-review H38). Skip the local lookup in cross-repo mode. If neither side exists → `skip: true, skipReason: "not-found"`. **Apply `isProtected` from Task 1** (no PR involved, so `isForkPr` and `verifyPrHeadIdentity` do not apply).

4. **Stale-open scan.** Apply gate helpers from Task 1 to every resolved PR source (`isForkPr`, `isProtected`, `verifyPrHeadIdentity` against the live remote head). **Mode-dependent behaviour** (plan-review H21):
...
_(Cross-cutting gates — fork detection, branch-reuse, protection — are now Task 1 helpers and are invoked from every candidate builder above. The earlier interstitial block describing them as a Task 5 retrofit has been removed; the helpers land before the candidate-mode tasks.)_

6. **Deduplication and source merge.** When the same branch arrives via multiple sources (e.g., `--pr 569 --branch feat/orphan --stale-days 30` covers the same branch twice), keep one action; OR the `needsPrClose` flag (any stale source requiring close wins); preserve explicit source metadata as an array `sources: CleanupSource[]` on the action.
...
7. **Local-branch metadata + worktree enumeration** (cross-repo: skipped — populate `local: null`, `worktrees: []`).
   ...
   - **Local branch identity check** (plan-review C44 + C2 — relocated here from old Task 1 since `action.local` is only populated in this step). For PR-sourced actions where `action.local.exists === true` AND `action.sources` contains a `type === "pr"` source with a known `prHeadSha`: compare `action.local.headSha` to that `prHeadSha` ONLY. **Do NOT accept `pr.merge_commit_sha` as proof of identity** — for squash/rebase merges, the merge commit is on `main`, not on the PR branch (plan-review C2). If `action.local.headSha !== prHeadSha` → `skip: true, skipReason: "local-branch-reused"`. Operators who explicitly named `--branch NAME` are exempt (they accepted the risk). Squash-merge recovery (`git update-ref refs/keep/<name> <branch>` before cleanup) is documented in the README. Add `preflight_local_branch_reused.test.ts`: `local.headSha === pr.head.sha` → proceed; `local.headSha === pr.merge_commit_sha` → skip `local-branch-reused`; unrelated commit → skip.

8. **Already-clean short-circuit** (plan-review H16/H25 — runs AFTER Task 7 for all candidate sources, AND AFTER `needsPrClose` evaluation). For each action whose `skip` is still `false` AND `needsPrClose === false` (a stale open PR still needs to be closed even when the branch is gone — plan-review codex:sequencing "already-clean can skip required stale PR closure"): if `remote.exists === false && (action.local === null || action.local.exists === false)` (the ref is gone everywhere and no local copy exists), mark `skip: true, skipReason: "already-clean"`. The empty action is preserved in the plan for visibility but execute treats it as no-op. The Phase 6 smoke runbook's third-run-exits-3 expectation depends on this: when all candidates are `already-clean`, `liveActionable === 0` → `EXIT_NOTHING_TO_DO`.

9. **Plan summary + write.** Populate `plan.summary.{candidates, actionable, skipped, skipReasons, truncated, dirtyWorktrees, localAhead, staleFiltered}`. Write plan via `writeCleanupPlan` into `cleanupDir()`. Before the write, call `runtime.ts:pruneOldPlans(30)` to garbage-collect stale plans (plan-review M16).
...
```bash
node --experimental-strip-types --test \
  plugins/stark-gh/tools/__tests__/cleanup/cleanup_gates.test.ts \
  ...
  plugins/stark-gh/tools/__tests__/cleanup/preflight_stale_filter_mode.test.ts \
  plugins/stark-gh/tools/__tests__/cleanup/preflight_stale_multi_pr.test.ts \
  ...
  plugins/stark-gh/tools/__tests__/cleanup/preflight_local_branch_reused.test.ts \
  plugins/stark-gh/tools/__tests__/cleanup/preflight_already_clean.test.ts \
...
10. **Audit-log helper body** (plan-review M25/H1/H6/H7/H12/H15/H28/H29/M1/M25/L0): fill the body of `runtime.ts:appendAuditLine(entry)` (signature already shipped in Phase 1 Task 4). The body owns lockfile acquisition, rotation, redaction, dropped-line counting — but NOT call sites; those are wired in Task 11 below.

    **Atomicity contract — dependency-free `O_CREAT | O_EXCL` lockfile** (no native binding required):
    - Acquire `<auditLog>.lock` via `fs.openSync(lockPath, "wx")` (atomic `O_CREAT | O_EXCL`). Lock file content: `{pid, hostname, startedAt}`. On `EEXIST`, retry every 50ms up to **2 s**. Stale-lock cleanup: if the lock file mtime is > **30 s** old AND `process.kill(pid, 0)` throws ESRCH, `unlink` and retry once.
    - **Lock acquisition failure after 2 s** → emit one stderr line `"AUDIT WARNING: lock held by pid <X> for >2s; skipping audit append for this action"`, increment `auditDropped`, and **continue without writing** (round-6 revision: refusing to run on contention is worse than dropping one audit line).
    ...
    - **Entry redaction** (plan-review M25 — `error.stderr` may carry tokens): every entry passes through `lib/redact.ts` before serialisation. `error.stderr` is redacted; branch names and PR titles are NOT redacted (user-visible identifiers).
    - **Rotation** (plan-review M1/L0): before each append, check audit log file size. If `> 10 MB` OR `mtime - createdAt > 90 days`, rotate via `fs.renameSync(auditLog, "<auditLog>.<YYYY-MM-DD>")`, then continue writing to a fresh `auditLog`. Keep the last **3** rotated files; older siblings removed by `pruneOldPlans` (Phase 1 Task 4 already handles `.audit.jsonl.YYYY-MM-DD`).

    `audit_log.test.ts` covers: single-write happy path; concurrent two-process writes (both lines land); stale-lock recovery (mocked `mtime - 65s` + pid not alive); lock-held-by-live-pid > 2 s → warning + `auditDropped++` + continue (NOT `EXIT_UNRECOVERABLE`); `ENOSPC` post-lock → warning + `auditDropped++` + continue; rotation at 10 MB and 90 days. `audit_log_redaction.test.ts`: synthetic action with token-bearing `error.stderr` → token NOT in the audit line.

11. **Audit-log call sites** (plan-review M15/H4 — wired across preflight, execute, and the wrapper). Each call site emits one JSON line via the Phase 1 Task 4 / Task 10 `appendAuditLine` helper:

    | Stage | Where (file) | When | Payload |
    |---|---|---|---|
    | `preflight` | `gh_cleanup_preflight.ts` end (Phase 3 Task 9, after `writeCleanupPlan`) | preflight completes (success path) | `{repo, flags, candidates, actionable, skipReasons, planFile, prunedPlans}` |
    | `wrapper-handoff` | `commands/cleanup.md` wrapper (Phase 5 Task 1, before `exec node ... execute`) — invoked via `gh_cleanup_audit_append.ts --stage wrapper-handoff` | wrapper has parsed envelope and is about to hand off to execute | `{planFile, dryRun, timestamp}` |
    | `action-start` | `gh_cleanup_execute.ts` action loop (Phase 4 Task 4, BEFORE the first Step of each action) | per-action, pre-steps | `{actionId, branch}` — lets post-hoc readers spot interrupted actions: action-start without matching action-end → interrupted (plan-review M15) |
    | `action` | `gh_cleanup_execute.ts` action loop (Phase 4 Task 4, AFTER every action's step completion — success, SkipAction, or StepFailed) | per-action, post-steps | `{actionId, branch, status, error?}` |
    | `execute` | `gh_cleanup_execute.ts` summary (Phase 4 Task 8, BEFORE plan-file unlink — ordering is mandatory per H14) | execute end-of-run | `{exitCode, counts, duration}` |
    | `cleanup-fail` | `gh_cleanup_execute.ts` summary (Phase 4 Task 8, when `fs.unlinkSync(planFile)` throws after a successful run — plan-review H14) | plan-file removal failed despite OK exit | `{planFile, error}` |

    Tests (each call site verified independently against a mocked `appendAuditLine`): `audit_log_actionstart.test.ts` (M15: action-start written before action-end), `wrapper_handoff_audit.test.ts` (H4: handoff line landed before `exec`), `execute_unlink_fail.test.ts` (H14: cleanup-fail line emitted on unlink throw + EXIT_OK preserved). The helper body (Task 10) is mocked in these tests so a single failure surface is exercised at a time.
...
| 3 | `cleanup_gates.test.ts` (Task 1 helpers: fork incl. null head.repo, branch-reuse incl. open-PR-on-same-head-ref query, protected case-fold), `preflight_pr.test.ts` (Task 2 matrix + gate-helper invocation in sequence), `preflight_open_pr.test.ts` (skip `pr-open`), `preflight_transient_gh_fail.test.ts` (non-404 → EXIT_UNRECOVERABLE), `preflight_branch.test.ts` (Task 3), `preflight_audit.test.ts` (M48 banner at preflight), `preflight_audit_truncated_no_results.test.ts` (H5: truncated + actionable=0 → EXIT_BAD_ARGS), `preflight_stale.test.ts`, `preflight_stale_filter_mode.test.ts` (H21: `--stale-days` with `--pr` does NOT trigger discovery scan), `preflight_stale_multi_pr.test.ts` (H22: two stale PRs on same head ref), `preflight_stale_sort.test.ts` (M26: oldest-first within cap), `preflight_stale_visibility.test.ts`, `preflight_worktree_scope.test.ts` (incl. H23 dirty main worktree triggers gate via `dirtyWorktreesForGate`), `preflight_dirty_auto_skip.test.ts`, `preflight_local_branch_reused.test.ts` (Task 7: pr.head.sha match → proceed; merge_commit_sha → skip; unrelated → skip), `preflight_already_clean.test.ts` (H16: runs after Task 7), `preflight_envelope.test.ts`, `preflight_dry_run.test.ts` (incl. M21 `--no-color` respected) |
| 4 | `execute_happy.test.ts`, `execute_revalidate.test.ts` (auth + cwd realpath + nodeId + protected-snapshot + detached-HEAD + protected-before-self-worktree ordering), `execute_live_local_recheck.test.ts` (H18: live-dirty / live-ahead), `execute_partial_failure.test.ts`, `execute_dry_run.test.ts`, `execute_worktree.test.ts` (incl. M6 existsSync re-check on remove failure), `execute_self_worktree.test.ts`, `execute_pr_close_failure.test.ts`, `execute_pr_close_retry.test.ts` (H7/H13/H25: re-fetch-then-act → "already-closed"), `execute_pr_already_closed.test.ts` (+ H3 branch-reused re-verify), `execute_freshness_changed.test.ts`, `execute_head_sha_drift.test.ts`, `execute_remote_gone.test.ts` (Step 1 404 → continue to Step 2; H17), `execute_retry_idempotent.test.ts` (M6 already-gone detection on remove + delete), `execute_cross_repo.test.ts`, `execute_truncated_banner.test.ts`, `execute_no_color.test.ts`, `execute_unlink_fail.test.ts` (H14: warn + EXIT_OK), `execute_multi_pr_close.test.ts` (H22: all PRs closed before remote delete), `stdout_json_shape.test.ts` (M4: all counter fields always present at zero), `audit_log.test.ts` (Task 10 helper body: concurrent writes; stale-lock recovery; lock-acquisition fail after 2 s → warning + auditDropped++ + continue per round-6 revision; rotation 10MB + 90d; ENOSPC → warning + auditDropped), `audit_log_redaction.test.ts` (Task 10: token-bearing error.stderr scrubbed), `audit_log_actionstart.test.ts` (Task 11: action-start written before action-end, M15) |
