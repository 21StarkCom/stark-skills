...
export function packet(run: Run, task: Assignment): string {
  ...
  return [
    ...
    "git merge-base --is-ancestor GRANTED_BASE HEAD and stop on failure. Regenerate, reconcile, rebuild, and retest.",
    "Push the rebased branch with an explicit force-with-lease. Fetch the PR head GitHub reports as PR_HEAD;",
    "require PR_HEAD to equal local HEAD, then check git merge-base --is-ancestor GRANTED_BASE PR_HEAD and stop on failure.",
    "Repost your review on any new head, then send Gru the final head SHA and review id in a progress report",
    "before merging, even if unchanged. Do not send ready while integrating: it is refused.",
    'For this progress report, message is a JSON string containing {"head":"<full PR head SHA>","review":<numeric review id>}.',
    "Check the posted review's commit_id equals PR_HEAD; stop on a mismatch before reporting or merging.",
    "Gru must receive that report and verify with the last reported head and review id; after merging",
    "nothing can repair missing ancestry or a review that does not cover the merged head.",
