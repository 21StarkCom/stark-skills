...
records a `worktree-adopted` event carrying the evidence. The worker's brief still
names the declared path, so send it a fresh `packet` before requiring intake.
