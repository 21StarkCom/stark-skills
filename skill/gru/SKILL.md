...
4. **Wait.** Minions report `done <PR> merged <sha>`, `blocked <reason>`, or
   ...
   closes its tab — and Phase 2 skips its now-`done` ticket. A dead peer is a
   death only while its ticket is still open. A dead Claude Minion with its
   ticket open is relaunched once with the same brief. A dead Codex Minion is a
   blocker:
