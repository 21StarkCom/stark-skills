...
**Config loading:** At the start of this step, check for `.github/project-config.json` in the target repo root. If the file does not exist, skip this entire step — project integration is opt-in.

**Auth: switch back to bot token.** Project field mutations require the GitHub App token:
...
**Error handling:** If a GraphQL call fails for a specific issue, log:
...
**Legacy labels:** The `sp:*`, `risk:*`, `confidence:*` labels continue to be created alongside project fields. This is additive migration — labels remain until the project workflow is validated.
