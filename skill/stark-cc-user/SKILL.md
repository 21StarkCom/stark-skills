...
- Setup commands print a `hint:` line pointing at the natural next step —
  `add` says the new profile is active-but-unplaced (use `order`), `order` points
  at `next`, and `next --dry-run` says how to actually switch. Hints are guidance,
  never warnings; pass them through with the rest of stdout.
