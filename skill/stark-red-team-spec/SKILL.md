...
argument-hint: "<spec-path> [--source-spec <path>] [--model <id>] [--dry-run] [--no-pr-comment] [--ready] [--fold]"
...
# our stderr, never into the challenge's stdout JSON (see Phase 2.1 step 7).
...
behind any flag (this skill has none — `--json` here is `red_team_spec.ts`'s
own flag, always passed in Phase 2.1).

- `outcome` is the challenge's own `status` (`clean` / `halted` /
  `halted_human_review` / `error`).
- `artifact_path` is `$spec_path` — durable (the sidecar was written and
...
if [ -n "$fold" ] && [ -s "$FOLD_STDERR_FILE" ]; then
  ...
  if [ -n "$fold_pr_number" ]; then
    ...
    echo "WARNING: stark-red-team-spec: fold step appears to have opened/edited a PR but its number could not be parsed from the fold summary (format may have drifted) — check $FOLD_STDERR_FILE" >&2
  fi
fi

cat <<EOF
STARK_STAGE_SUMMARY {"skill":"stark-red-team-spec","outcome":"$status","artifact_path":$ARTIFACT_JSON,"pr":$PR_JSON,"fold_prs":$fold_prs_json}
EOF
```
