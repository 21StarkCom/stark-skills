...
description: >-
  ...
  xhigh reasoning) reviews 5 adversarial domains in parallel; Claude (opus-5 1m)
...
- **Wing fixer** receives the plan + classified `fix` findings and emits a JSON
  ...
  applies surgically. Default agent is claude (opus-5 1m); `--wing-agent codex`
...
print_completion_line() {
    ...
    cat <<EOF
STARK_STAGE_SUMMARY {"skill":"stark-review-plan","outcome":"$OUTCOME","artifact_path":$ARTIFACT_JSON,"pr":$PR_JSON}
EOF
}
```
...
headless with no operator to ask — set `OUTCOME="review_failed"`, call
`print_completion_line` (Phase 7), and exit 1.
...
`converged_*` outcomes — the plan's changes are durable on a PR; every other
