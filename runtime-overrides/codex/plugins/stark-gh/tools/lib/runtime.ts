...
function stateRoot(): string {
  const configured = process.env.STARK_STATE_ROOT?.trim();
  return configured || path.join(os.homedir(), ".stark", "code-review");
}

export function runtimeDir(): string {
  return path.join(stateRoot(), "stark-gh", "runtime");
}
