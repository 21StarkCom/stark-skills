#!/usr/bin/env node
// Watcher --on-green callback for $pr-merge.
...
function buildRunbook(plan: PrMergePlan): OperatorRunbook {
  return {
    ...
    cleanup_command: `$cleanup --pr ${plan.pr.number}`,
    branch_recovery_window: "until $cleanup runs (then GC window)",
  };
}
...
async function main(argv: string[]): Promise<number> {
  ...
  writeTerminalState(plan, "merged", {
    mergeSha,
    cleanupHint: `Run: $cleanup --pr ${plan.pr.number}`,
  });
