...
export function reverifyState(plan: Plan, opts: { exec?: ExecFn } = {}): void {
  ...
  if (!fingerprintsMatch(plan.stateFingerprint, actual)) {
    ...
    throw new Error(`state changed between preflight and execute (${fields}); rerun $pr-open`);
  }
}
...
function main(): never {
  ...
  if (plan.stage3.action !== "push-only") {
    ...
    if (fresh.baseOid !== plan.baseOid) {
      die(
        Exit.BASE_OID_DRIFT,
        `base branch moved upstream (was ${plan.baseOid}, now ${fresh.baseOid}); rerun $pr-open`,
      );
    }
  }
