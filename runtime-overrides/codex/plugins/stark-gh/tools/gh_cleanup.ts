...
import { mayDeleteLocalBranch, shouldFetchForPlan } from "./cleanup_policy.ts";
...
export function buildPlanFullSweep(args: CleanupArgs): CleanupPlan {
  ...
  const notes: string[] = [];
  // Fetch + prune is best-effort during a real run, but forbidden during a dry
  ...
  if (shouldFetchForPlan(args)) {
    ...
    notes.push("dry-run: fetch + prune skipped to preserve all local refs");
  }
  ...
  const willDelete = new Set(
    localBranches
      .filter(b => mayDeleteLocalBranch(b, args))
      .map(b => b.name),
  );
...
export function buildPlanSinglePr(prNumber: number, args: CleanupArgs): CleanupPlan {
  ...
  const willDelete = new Set(
    localBranches.filter(b => mayDeleteLocalBranch(b, args)).map(b => b.name),
  );
  const notes: string[] = [];
  const worktrees: WorktreePlan[] = [];
  for (const w of listWorktrees()) {
...
export function executePlan(plan: CleanupPlan, args: CleanupArgs): ExecuteReceipt {
  ...
  for (const b of plan.localBranches) {
    if (!mayDeleteLocalBranch(b, args)) {
